<?php

namespace Core\DAO {
    use \PDO as PDO;
    use Exception as Exception;
    use \Core\Util;

    abstract class DataManipulationLanguage {
        private $field;
        private $related;
        private $limit;
        private $order_by;
        private $primary_key;
        private $last_insert_id;

        public function __construct() {
            $this->clearData();
        }

        private function getTableName() {
            return $this->name();
        }

        private function getTableColumn() {
            return $this->column();
        }

        private function getTableSchema() {
            return $this->schema();
        }

        private function getField() {
            return $this->field;
        }

        private function setField($value) {
            $this->field = $value;
        }

        private function getRelated() {
            return $this->related;
        }

        private function setRelated($value) {
            $this->related = $value;
        }

        private function getLimit() {
            return $this->limit;
        }

        private function setLimit($limit) {
            $this->limit = $limit;
        }

        private function getOrderBy() {
            return $this->order_by;
        }

        private function setOrderBy($value) {
            $this->order_by = $value;
        }

        private function getPrimaryKey() {
            return $this->primary_key;
        }

        private function setPrimaryKey($column) {
            $this->primary_key = $column;
        }

        private function getLastInsertId() {
            return $this->last_insert_id;
        }

        private function setLastInsertId($id) {
            $this->last_insert_id = $id;
        }

        private function clearData() {
            $this->select($field = null);
            $this->orderBy($order_by = []);
            $this->limit($page = 1,$limit = QUERY_LIMIT_ROW);
            $this->definePrimaryKey($column = "id");
        }

        private function definePrimaryKey($column = null) {
            $table_schema = $this->schema();

            foreach ($table_schema as $i => $value) {
                if ($value->method == "primaryKey") {
                    $column = $i;
                    break;
                }
            }

            $this->setPrimaryKey($column);
        }

        public function select($field = []) {
            if (!empty($field)) {
                $this->setField($field);

            } else {
                $field = $this->getTableColumn();
                $table_name = $this->getTableName();
                $field_list = [];

                foreach ($field as $i => $value) {
                    $field_list[] = vsprintf("%s.%s %s__%s",[$table_name,$i,$table_name,$i]);

                }

                $this->setField($field_list);
            }
        }

        public function orderBy($order_by = []) {
            if (!empty($order_by)) {
                $order_by_list = [];

                foreach ($order_by as $i => $value) {
                    $order_by_list[] = vsprintf("%s %s",[$i,$value]);
                }

                $order_by_list = implode(",",$order_by_list);
                $order_by_list = vsprintf("order by %s",[$order_by_list,]);

                $this->setOrderBy($order_by_list);
            }

            return $this;
        }

        public function limit($page = 1,$limit = QUERY_LIMIT_ROW) {
            $limit_value = null;

            if ($page <= 1) {
                $limit_value = vsprintf("limit %s offset 0",[$limit,]);

            } else {
                $page -= 1;
                $page_x_limit = $page * $limit;
                $limit_value = vsprintf("limit %s offset %s",[$limit,$page_x_limit]);
            }

            $this->setLimit($limit_value);

            return $this;
        }

        // public function related($foreign_Key = null,$field = [],$list = []) {
        //     $table_name = $this->getTableName();
        //     $table_primary_key = $this->getTablePrimaryKey();
            
        //     if (!empty($foreign_Key)) {
        //         $table_foreign_Key = $foreign_Key->getTableForeignKey();

        //     } else {
        //         $table_foreign_Key = $this->getTableForeignKey();
        //     }

        //     if (!empty($table_foreign_Key)) {
        //         $list = [];

        //         foreach ($table_foreign_Key as $key => $value) {
        //             $table_index = $key;
        //             $foreign_Key = $value["index"];
        //             $field = array_merge($field,$foreign_Key->getField());

        //             $table_join = $foreign_Key->name();
        //             $table_join_index = $foreign_Key->primaryKey();
        //             $table_join_index_null = $value["null"];

        //             $join_type = "inner";

        //             if (!empty($table_join_index_null)) {
        //                 $join_type = "left";
        //             }

        //             $list[] = vsprintf("%s join %s on %s.%s=%s.%s",[$join_type,$table_join,$table_join,$table_join_index,$table_name,$table_index]);

        //             $get_table_foreign_key = $foreign_Key->getTableForeignKey();

        //             if (!empty($get_table_foreign_key)) {
        //                 $this->related($foreign_Key,$field,$list);
        //             }
        //         }

        //         $this->setField(array_merge($this->getField(),$field));

        //         $value = implode(" ",$list);
        //         $this->setRelated($value);
        //     }

        //     return $this;
        // }

        // public function exclude($exclude = []) {
        //     if (!empty($exclude)) {
        //         $list = [];
                
        //         foreach ($exclude as $key => $value) {
        //             $this->setExcludeEscape($value);
                    
        //             $list[] = vsprintf("%s not in(?)",[$key,]);
        //         }

        //         $list = implode(" and ",$list);

        //         $this->setExclude($list);
        //     }

        //     return $this;
        // }

        public function filter($where = []) {
            if (empty($GLOBALS["TRANSACTION"])) {
                throw new Exception("Transaction don't initiated.");
            }

            $table_column = $this->getTableColumn();
            $table_name = $this->getTableName();
            $table_schema = $this->getTableSchema();

            $order_by = $this->getOrderBy();
            $limit = $this->getLimit();
            $field = $this->getField();

            $query = vsprintf("select %s from %s",[$field,$table_name]);

            $sql_value_list = [];

            if (empty($where)) {
                $query = vsprintf("%s %s %s",[$query,$order_by,$page]);

            } else {
                $where_escape = [];

                foreach ($where as $key => $value) {
                    $value_escape_list = [];

                    foreach ($value as $sub_value) {
                        $value_escape_list[] = "?";
                        $sql_value_list[] = $sub_value;
                    }

                    $value_escape_list = implode(",",$value_escape_list);

                    $where_escape[] = vsprintf("%s in(%s)",[$key,$value_escape_list]);
                }

                $where_escape = implode(" and ",$where_escape);
                $where_escape = vsprintf("where %s",[$where_escape,]);

                $sql_consult_id .= vsprintf(" %s",[$where_escape]);
                $query .= vsprintf(" %s %s %s",[$where_escape,$order_by,$page]);
            }

            try {
                $statement = $link->prepare($query);
                $statement->execute($sql_value_list);
                $result = $statement->fetchAll(PDO::FETCH_CLASS);

            } catch (Exception $error) {
                $this->clearData();

                throw new Exception($error);
            }

            $this->clearData();

            $this->setQueryValue($result);
            $this->setUpdateFilterSqlEscape($sql_consult_id);
            $this->setUpdateFilterSqlValueList($sql_value_list);

            return $this;
        }

        public function get($where = []) {
            if (empty($GLOBALS["TRANSACTION"])) {
                throw new Exception("Transaction don't initiated.");
            }

            if (empty($where)) {
                throw new Exception("error in get, where don't set");
            }

            $table_column = $this->getTableColumn();
            $table_name = $this->getTableName();
            $table_schema = $this->getTableSchema();

            $where_escape = [];
            $where_value = [];

            foreach ($where as $i => $value) {
                if (!array_key_exists($i,$table_column)) {
                    throw new Exception("field missing, check our model");
                }

                if (!array_key_exists($i,$table_schema)) {
                    throw new Exception("field missing, check our schema");
                }

                $where_escape[] = vsprintf("%s.%s=?",[$table_name,$i]);
                $where_value[] = $value;
            }

            $where = vsprintf("where %s",[implode(" and ",$where_escape),]);
            $field = [];

            foreach ($table_column as $i => $value) {
                $field[] = vsprintf("%s.%s",[$table_name,$i]);
            }

            $field = implode(",",$field);

            $query_total = vsprintf("select count(1) total from %s %s",[$table_name,$where]);
            $query = vsprintf("select %s from %s %s",[$field,$table_name,$where]);

            try {
                $query_total = $GLOBALS["TRANSACTION"]->getResource()->prepare($query_total);
                $query_total->execute($where_value);
                $query_total = $query_total->fetch(PDO::FETCH_OBJ);

                if ($query_total->total != 1) {
                    throw new Exception("error in get, don't unique result");
                }

                $query = $GLOBALS["TRANSACTION"]->getResource()->prepare($query);
                $query->execute($where_value);
                $query = $query->fetch(PDO::FETCH_OBJ);

            } catch (Exception $error) {
                throw new Exception($error);
            }

            foreach ($query as $i => $value) {
                $this->$i = $value;
            }

            $this->clearData();

            return $this;
        }

        public function save($field = null) {
            if (empty($GLOBALS["TRANSACTION"])) {
                throw new Exception("Transaction don't initiated.");
            }

            $table_name = $this->getTableName();
            $table_column = $this->getTableColumn();
            $table_schema = $this->getTableSchema();

            if (!empty($field)) {
                $column = [];
                $row = [];
                $row_escape = [];

                foreach ($field as $i => $value) {
                    if (!array_key_exists($i,$table_column)) {
                        throw new Exception("field missing, check our model");
                    }

                    if (!array_key_exists($i,$table_schema)) {
                        throw new Exception("field missing, check our schema");
                    }

                    $method = $table_schema[$i]->method;
                    $rule = $table_schema[$i]->rule;

                    try {
                        $this->$method(
                            $rule = $rule,
                            $value = $value,
                            $flag = true);

                    } catch (Exception $error) {
                        throw new Exception($error);
                    }

                    $column[] = $i;
                    $row[] = $value;
                    $row_escape[] = "?";
                }

                $column = implode(",",$column);
                $row_escape = implode(",",$row_escape);

                $query = vsprintf("insert into %s (%s) values(%s)",[$table_name,$column,$row_escape]);

            } else {
                $row = [];
                $set_escape = [];

                foreach ($table_column as $i => $value) {
                    $set_escape[] = vsprintf("%s=?",[$i,]);
                    $row[] = $value;
                }

                $set_escape = implode(",",$set_escape);

                $primary_key = $this->getPrimaryKey();
                $last_insert_id = $this->getLastInsertId();

                $where = vsprintf("%s=%s",[$primary_key,$last_insert_id]);

                $query = vsprintf("update %s set %s where %s",[$table_name,$set_escape,$where]);
            }

            try {
                $query = $GLOBALS["TRANSACTION"]->getResource()->prepare($query);
                $query->execute($row);

            } catch (Exception $error) {
                throw new Exception($error);
            }

            $query_error_info = $query->errorInfo();

            if ($query_error_info[0] != "00000") {
                throw new Exception($query_error_info[2]);
            }

            $get_database_info = $GLOBALS["TRANSACTION"]->getDatabaseInfo();

            $sequence_name = null;

            if ($get_database_info["DB_DRIVER"] == "pgsql") {
                $sequence_name = vsprintf("%s_id_seq",[$table_name,]);
            }

            $last_insert_id = $GLOBALS["TRANSACTION"]->lastInsertId($sequence_name = $sequence_name);

            $this->setLastInsertId($id = $last_insert_id);
            $this->get($where = ["id" => $last_insert_id]);
            $this->clearData();

            return $this;
        }

        public function delete($field = []) {
            if (empty($GLOBALS["TRANSACTION"])) {
                throw new Exception("Transaction don't initiated.");
            }

            $table_name = $this->getTableName();
            $table_column = $this->getTableColumn();
            $table_schema = $this->getTableSchema();

            if (!empty($field)) {
                $column = [];
                $row = [];
                $row_escape = [];

                foreach ($field as $i => $value) {
                    if (!array_key_exists($i,$table_column)) {
                        throw new Exception("field missing, check our model");
                    }

                    if (!array_key_exists($i,$table_schema)) {
                        throw new Exception("field missing, check our schema");
                    }

                    $method = $table_schema[$i]->method;
                    $rule = $table_schema[$i]->rule;

                    try {
                        $this->$method(
                            $rule = $rule,
                            $value = $value,
                            $flag = true);

                    } catch (Exception $error) {
                        throw new Exception($error);
                    }

                    $where[] = vsprintf("%s=?",[$i,]);
                    $row[] = $value;
                }

                $where = implode(",",$where);

                $query = vsprintf("delete from %s where %s",[$table_name,$where]);

            } else {
                $row = [];

                foreach ($table_column as $i => $value) {
                    $this->$i = null;
                }

                $primary_key = $this->getPrimaryKey();
                $last_insert_id = $this->getLastInsertId();

                $where = vsprintf("%s=%s",[$primary_key,$last_insert_id]);

                $query = vsprintf("delete from %s where %s",[$table_name,$where]);
            }

            try {
                $query = $GLOBALS["TRANSACTION"]->getResource()->prepare($query);
                $query->execute($row);

            } catch (Exception $error) {
                throw new Exception($error);
            }

            $query_error_info = $query->errorInfo();

            if ($query_error_info[0] != "00000") {
                throw new Exception($query_error_info[2]);
            }

            $get_database_info = $GLOBALS["TRANSACTION"]->getDatabaseInfo();

            $sequence_name = null;

            if ($get_database_info["DB_DRIVER"] == "pgsql") {
                $sequence_name = vsprintf("%s_id_seq",[$table_name,]);
            }

            $last_insert_id = $GLOBALS["TRANSACTION"]->lastInsertId($sequence_name = $sequence_name);

            $this->clearData();

            return $this;
        }

        // public function update($set) {
        //     $table_name = $this->getTableName();
        //     $link = $this->getLink();
        //     $update_filter_sql_escape = $this->getUpdateFilterSqlEscape();
        //     $update_filter_sql_value_list = $this->getUpdateFilterSqlValueList();

        //     if (empty($set)) {
        //         $this->clearData();

        //         throw new Exception("param missing");
        //     }

        //     if (empty($update_filter_sql_escape) || empty($update_filter_sql_value_list)) {
        //         $this->clearData();

        //         throw new Exception("param missing");
        //     }

        //     $set_escape = [];
        //     $set_value_list = [];

        //     foreach ($set as $key => $value) {
        //         $set_escape[] = $table_name.".".$key."='".$value."'";
        //     }

        //     $set_escape = implode(",",$set_escape);

        //     try {
        //         $sql = $update_filter_sql_escape;
        //         $prepare = $link->prepare($sql);
        //         $prepare->execute($update_filter_sql_value_list);
        //         $result = $prepare->fetchAll(PDO::FETCH_BOTH);

        //     } catch (Exception $error) {
        //         $this->clearData();

        //         throw new Exception($error);
        //     }

        //     if (empty($result)) {
        //         $this->clearData();
  
        //         throw new Exception("query is empty for update.");
        //     }

        //     $id_list = [];

        //     foreach ($result as $key => $value) {
        //         $id_list[] = $value["id"];
        //     }

        //     $id_list = implode(",",$id_list);

        //     try {
        //         $sql = "update ".$table_name." set ".$set_escape." where ".$table_name.".id in(".$id_list.")";
        //         $prepare = $link->prepare($sql);
        //         $prepare->execute();
        //         $result = $prepare->rowCount();
  
        //     } catch (Exception $error) {
        //         $this->clearData();

        //         throw new Exception($error);
        //     }

        //     $this->clearData();
        //     $this->setUpdateResult($result);

        //     return $this;
        // }

        public function __destruct() {
            unset($this);
        }
    }
}

?>